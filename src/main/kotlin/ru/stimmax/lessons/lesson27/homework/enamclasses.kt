...
enum class TrafficLightSignal(val color: Colors, val duration: Long, val blinkTimes: Int? = null) {
    GREEN(Colors.GREEN_COLOR, 10),
    GREENFLASH(Colors.GREEN_COLOR, 10, 3),
    YELLOW(Colors.YELLOW_COLOR, 5),
    RED(Colors.RED_COLOR, 15)
    ;
}
...
fun FlashLight() {
    ...
    var flash: Unit
    while (counter < 10) {
        flash = showSignal(TrafficLightSignal.GREEN)
        clear()
        flash =
            TrafficLightSignal.GREENFLASH.blinkTimes?.let {
                repeat(it) {
                    showSignal(TrafficLightSignal.GREENFLASH, 200)
                    clear()
                }
            }!!
        flash = showSignal(TrafficLightSignal.YELLOW)
        clear()
        flash = showSignal(TrafficLightSignal.RED)
        clear()
        counter++
        println(flash)
    }
...
enum class CoffeeType(
    ...
    NESCAFEE_CLASSIC(2.15, "Nescafe classic", 0);

    fun description() =
        "The $coffeeBase ${if (hasMilk) "with" else "without"} milk costs \$$cost and is best with $recommendedSugar spoons of sugar."
}

enum class CharacterTypes(val profession: String) {
    CTO("СТО"),
    UX_UI("дизайнер"),
    CRM(" CRM Менеджер"), // Customer Relationship Manager
    GAME_DEV("геймдев"),
    TEAM_LEAD("тимлид"),
    BACKEND_DEV("бэкендер"),
    PM("Продакт Менеджер"),
    SYSADMIN("сисадмины"),
    QA("QA")
}
...
fun typeOfCoffee(coffeeType: CoffeeType) {
    val type = when (coffeeType) {
        CoffeeType.ESPRESSO -> "${CharacterTypes.CTO} or ${CharacterTypes.TEAM_LEAD}"
        CoffeeType.LATTE -> "${CharacterTypes.UX_UI} and ${CharacterTypes.GAME_DEV}"
        CoffeeType.CAPPUCCINO -> " ${CharacterTypes.PM} and ${CharacterTypes.CRM} "
        CoffeeType.AMERICANO -> "${CharacterTypes.BACKEND_DEV} and ${CharacterTypes.SYSADMIN}"
        CoffeeType.NESCAFEE_CLASSIC -> "${CharacterTypes.QA}"
    }
    println(type)
}